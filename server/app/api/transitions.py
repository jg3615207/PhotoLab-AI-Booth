...
def list_transitions():
    with get_db() as db:
        rows = db.execute("SELECT * FROM transitions ORDER BY created_at DESC").fetchall()
...
def create_transition(
    ...
    with get_db() as db:
        ...
        db.execute(
            "INSERT INTO transitions (id, name, duration, css_code, active) VALUES (?,?,?,?,1)",
            (id_clean, name, duration, css_code),
        )
...
def update_transition(
    ...
    with get_db() as db:
        ...
        db.execute(
            "UPDATE transitions SET name=?, duration=?, css_code=?, active=? WHERE id=?",
            (name, duration, css_code, active, tid),
        )
...
def get_public_transitions():
    with get_db() as db:
        rows = db.execute("SELECT id, name, duration FROM transitions WHERE active=1").fetchall()
        return [dict(r) for r in rows]
